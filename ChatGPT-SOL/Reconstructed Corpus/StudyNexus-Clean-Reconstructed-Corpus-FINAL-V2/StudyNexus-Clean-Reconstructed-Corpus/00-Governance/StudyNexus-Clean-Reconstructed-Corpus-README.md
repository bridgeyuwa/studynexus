...
This is the subject-oriented working knowledge base derived from the 555-file historical archive and the accepted current Product Discovery authority. Its active `COR-*` dossiers are explicitly split into source-grounded reconstructions and current-authority overlays. It removes exact-copy noise from active use, groups evolving/near-duplicate evidence, separates current support from historical technical proposals, and preserves source-level provenance through stable IDs and ledgers.
...
Document kind describes **where a dossier gets its evidentiary grounding**; it is separate from temporal/status classification.

- `SOURCE_RECONSTRUCTION` — a subject dossier genuinely reconstructed from one or more original historical `SRC-*` sources. It must have non-zero source lineage with hashes.
- `AUTHORITY_OVERLAY` — a current navigation/support dossier derived from the accepted Product Discovery and/or its final supporting artifacts. It intentionally has no historical `SRC-*` lineage and must identify its authority sources explicitly.

`CURRENT_SUPPORTING` remains a status. `AUTHORITY_OVERLAY` does not replace or alter that status.

## ID scheme

- `COR-*`: active subject dossier. Each dossier declares `document_kind: SOURCE_RECONSTRUCTION` or `document_kind: AUTHORITY_OVERLAY`.
...
Every original file has exactly one `SRC-*` row in the Source Disposition Ledger. Every `SOURCE_RECONSTRUCTION` dossier identifies real `SRC-*` lineage and source hashes. An `AUTHORITY_OVERLAY` has `source_count: 0`, claims no historical `SRC-*` provenance, and instead identifies the accepted Product Discovery/final supporting artifacts in `authority_sources`. Exact duplicate copies map to a representative source rather than being treated as independent evidence.
...
Give the agent the Current Authority Map and the relevant subject dossiers, not the entire historical archive by default. Tell it to inspect each `COR-*` dossier's `document_kind`: `SOURCE_RECONSTRUCTION` means historical source-grounded evidence; `AUTHORITY_OVERLAY` means current navigation/support grounded in the accepted Product Discovery/final artifacts rather than `SRC-*` evidence. The accepted Product Discovery handoff remains the authority for current product meaning. Historical `SRC-*` evidence should be consulted when provenance, evolution or a disputed statement matters.
