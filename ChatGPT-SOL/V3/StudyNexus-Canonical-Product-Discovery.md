...
**Status:** Current authoritative product-discovery baseline  
...
| CAP027 | Internationalization Semantics | MVP — defined | partially defined | FOUNDATIONAL_PRODUCT_PRINCIPLE | HIGH | C038 C051 C057 C084 D005 R014 |
...
All capabilities with a current scope status beginning **MVP —** belong to the strategic MVP even when their definition is incomplete. Capabilities marked **CURRENTLY DEFERRED** have positive deferral evidence. Capabilities marked **EXCLUDED** have evidence-backed product-boundary exclusions. **GENUINELY UNRESOLVED WHETHER PRODUCT CAPABILITY** means the evidence cannot yet establish product membership.
...
| CON056 | Ranking / Institutional Intelligence | A possible product representation of comparative institutional metrics, imported rankings, normalized observations or StudyNexus-derived intelligence. | GENUINELY UNRESOLVED WHETHER PRODUCT CAPABILITY | genuinely unresolved | LOW |
...
| CON058 | Organization (potential shared abstraction) | A possible umbrella abstraction for non-person actors such as regulators, exam bodies, scholarship providers, publishers or service providers. The corpus does not yet prove that these actors share one product meaning. | MVP — boundary requires refinement | genuinely unresolved | LOW-MEDIUM |
...
- jurisdiction-aware internationalization semantics;
...
## A002 — Current-MVP normalization rule

**Assumption/owner instruction:** capabilities still under current consideration remain in strategic MVP unless explicit evidence establishes current exclusion or deferral. Historical delivery phases do not decide present membership.

**Evidence:** current productionization protocol, D021, R004.

**Downstream impact:** scope interpretation, not domain semantics.
...
- Current-MVP normalization rule as supplied by product owner.
...
# Part XVII — Readiness Gate for Ubiquitous Language
...
| Programme/offering terminology readiness | READY WITH EXPLICIT ASSUMPTION | Institution Offering and Discovery Programme are separated; ProgrammeInstance remains historical technical vocabulary. |
...
| Assumption visibility | READY | Three canonicalization assumptions are isolated and traceable. |
| Evidence traceability | READY | Major conclusions use C/D/R identifiers and registers preserve source paths. |
| Absence of unresolved true blockers | READY | No remaining issue prevents UL; unresolved ranking/social membership can remain outside vocabulary-core decisions. |
...
StudyNexus Product Discovery is sufficiently coherent, current-scope explicit, evidence-traceable and product/architecture-separated to begin Ubiquitous Language work.

There is **no unresolved true Product Discovery blocker**.

The most important assumption to carry visibly into the next stage is the **Institution Offering / Discovery Programme distinction**, including optional classification and unresolved offering-context vocabulary. The accounts/personalization/notification interpretation is also a current-scope normalization assumption, but it does not block core vocabulary work.

Rankings/institutional intelligence and a full social/community product remain genuine product-membership unknowns. They should not be forced into the Ubiquitous Language core merely to make the vocabulary exhaustive.

Several current-MVP capabilities remain underspecified. That is acceptable because their uncertainty is explicit, bounded and independent of MVP membership.
